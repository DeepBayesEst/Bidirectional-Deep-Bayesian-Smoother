...
class LSTMCell(LayerRNNCell):
  """Long short-term memory_normal unit (LSTM) recurrent network cell.
  ...
  "Long short-term memory_normal recurrent neural network architectures for
  ...
  def call(self, inputs, state):
    ...
    lstm_matrix_m = math_ops.matmul(array_ops.concat([c_prev, m_prev[:,:2]/30000,m_prev[:,2:]/10], 1), self._kernel_mem)
    c_update = sigmoid(nn_ops.bias_add(lstm_matrix_m, self._bias_mem))

    lstm_matrix_mod_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev[:,:2]/30000,m_prev[:,2:]/10], 1), self._kernel_mod_l1)
    ...
    lstm_q_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev[:,:2]/30000,m_prev[:,2:]/10], 1), self._kernel_q_l1)
    ...
    P_pred = math_ops.matmul(math_ops.matmul(self._trans_model,P_prev),transpose(self._trans_model,[0,2,1])) \
             +self._pro_noise + Q_est##


    v = reshape(inputs,[self._batch_size,self._dim_meas,1]) - math_ops.matmul(self._meas_model,m_pred) # self._meas_func(m_pred)

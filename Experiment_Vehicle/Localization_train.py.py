...
#
train_step1 = tf.train.AdamOptimizer(learing_rate1).minimize(loss_pos, global_step=global_steps1, var_list=loss_vars1)
train_step2 = tf.train.AdamOptimizer(learning_rate2).minimize(smooth_loss_pos, global_step=global_steps2)
...
with tf.Session() as sess:
    ...
    for step in range(20000):
        ...
        P_start_ = np.stack(1*np.array([np.eye(4) for _ in range(batch_size)], "float64"))
        #
        _,loss_print,resu,loss_0 = sess.run([train_step1,loss11,M_arr,loss0],feed_dict={x:meas_batch,y_:state_batch[:,1:,:],h_start:h_start_in,c_start:c_start_,P_start:P_start_})
        _, loss_pinghua = sess.run([train_step2, smooth_loss11],feed_dict={x:meas_batch,y_:state_batch[:,1:,:],h_start:h_start_in,c_start:c_start_,P_start:P_start_})

        if step % 100 == 0:
            print('**************************************************')
            print('Train step %d'%step)
            print('Filter loss：',np.sqrt(loss_print))
            print('Smooth loss：', np.sqrt(loss_pinghua))
            print('Measurement noise:',np.sqrt(np.mean(np.square(state_batch[:,:,2:]-meas_batch))))
            print('**************************************************')
        # if step % 1000 == 0:
        #     saver.save(sess, "./Car/model.ckpt")

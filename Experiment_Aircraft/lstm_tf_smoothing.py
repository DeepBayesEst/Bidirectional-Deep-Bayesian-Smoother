...
class BasicLSTMCell(LayerRNNCell):
  ...
  def call(self, inputs, state):
    """Long short-term memory_normal cell (LSTM).
...
def get_sinusoidal_encoding(seq_len, embed_dim):
    # Create a column vector of position indices from 0 to seq_len-1, shape: [seq_len, 1]
    position = np.arange(seq_len)[:, np.newaxis]           # [seq_len, 1]
    div_term = np.exp(np.arange(0, embed_dim, 2) * -np.log(10000.0) / embed_dim)
    pe = np.zeros((seq_len, embed_dim))
    pe[:, 0::2] = np.sin(position * div_term)              # even indices
    pe[:, 1::2] = np.cos(position * div_term)              # odd indices
    return tf.constant(pe, dtype=tf.float32)               # [seq_len, embed_dim]

class CrossAttention(tf.keras.layers.Layer):
  '''
  Cross-attention mechanism for bidirectional memory fusion
  '''
  ...
  def call(self, x1, x2,x3):
      ...
      # Split heads and compute attention_lessattention per head
...
class LSTMCell(LayerRNNCell):
  ...
  def __init__(self, num_units,dim_state,dim_meas,
    ...
    self._forget_bias = forget_bias
    self._state_is_tuple = state_is_tuple
    # Prior information related to the smoother
  ...
  def build(self, inputs_shape):
    ...
    self._kernel_g_l1 = self.add_variable("kernel_g_l1", shape=[state_depth + h_depth, self._num_units],
  ...
  def call(self, input_smooth, state):
      # Get the filtering result and the smoothing result of the previous step
      ...
      '''
      Backward memory update gate (BMUG)
      '''
      c_attention = sigmoid(self.cro_attn(array_ops.concat([m_prev[:,:2]/100000,m_prev[:,2:]/100], 1), C_filter,c_prev))
      '''Backward smoothing gate (BSG)'''
      # Backward measure
      lstm_g_l1 = math_ops.matmul(array_ops.concat([c_attention, m_prev[:, :2] / 100000, m_prev[:, 2:] / 100], 1),
                                  self._kernel_g_l1)
      ...
      lstm_g_l1_2 = math_ops.matmul(array_ops.concat([c_attention, m_prev[:, :2] / 100000, m_prev[:, 2:] / 100], 1),
                                    self._kernel_g_l1_2)
      ...
      '''
      m_buchang = m_p + add
      AT = tf.transpose(self._trans_model, perm=[0, 2, 1])
      P_buchang = P_p + P_est
      P_p_inverse = tf.linalg.inv(P_buchang)
      # smoothing gain
      G_s = math_ops.matmul(math_ops.matmul(P_f, AT), P_p_inverse)
      m_s = reshape(m_f, [self._batch_size, self._dim_state, 1]) + math_ops.matmul(G_s, (
                  reshape(m_smooth, [self._batch_size, self._dim_state, 1]) - m_buchang))
      G_s_T = tf.transpose(G_s, perm=[0, 2, 1])
      P_s = P_f + math_ops.matmul(math_ops.matmul(G_s, (P_prev - P_buchang)), G_s_T)
      m_s = reshape(m_s, [self._batch_size, self._dim_state])
      c = c_attention
      new_state = (LSTMStateTuple_KF(c, m_s, P_s) if self._state_is_tuple else
                   array_ops.concat([c, m_s, P_s], 1))

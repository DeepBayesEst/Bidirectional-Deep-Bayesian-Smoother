...
class LSTMCell(LayerRNNCell):
  """
  Our filtering model.
  This model is written based on the basic LSTM class.
  """
  ...
  def build(self, inputs_shape):
    ...
    '''Defining neural network weights'''
    ...
    if self.dtype is None:
      ...
      initializer = init_ops.zeros_initializer(dtype=self.dtype)
    '''Defining neural network biases'''
  ...
  def call(self, inputs, state):
    ...
    '''
    Forward memory update gate (FMUG)
    '''
    lstm_matrix_m = math_ops.matmul(array_ops.concat([c_prev, m_prev[:,:2]/100000,m_prev[:,2:]/100], 1), self._kernel_mem) # 之前的数据：100000和1000
    c_update = sigmoid(nn_ops.bias_add(lstm_matrix_m, self._bias_mem))
    '''
    Forward filter gate (FFG)
    '''
    # Forward Compensation
    lstm_matrix_mod_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev[:,:2]/100000,m_prev[:,2:]/100], 1), self._kernel_mod_l1)
    ...
    # Forward measure
    lstm_q_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev[:,:2]/100000,m_prev[:,2:]/100], 1), self._kernel_q_l1)
    ...
    S = math_ops.matmul(math_ops.matmul(H_mat,P_pred),
                        transpose(H_mat,[0,2,1])) + self._meas_noise
    # Kalman gain
